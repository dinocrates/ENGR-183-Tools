...
**T3.15 — Light/dark theme (added, not in original plan)**
Stephen: add light/dark, with high contrast (a genuine accessibility mode) scoped as a deliberate follow-up rather than bundled in, given the size of doing HC properly — real WCAG contrast-ratio work, not just a third palette. Starting point was a full repo scan: zero theme abstraction existed (confirmed via an Explore pass) — ~90 hardcoded `slate-*`/`cyan-*`/`red-*` Tailwind classes across all 14 components, no `@theme` block, no CSS custom properties, `index.css` hardcoding `color-scheme: dark`.
Built a 15-token semantic layer in `src/index.css` (`@theme` block, e.g. `--color-surface`, `--color-primary`, `--color-accent-fg`) with dark values as defaults and light values overriding under `:root[data-theme="light"]` — ordinary CSS cascade, so every Tailwind utility generated from a token (`bg-surface`, `text-primary`, ...) picks up the override with zero per-component branching. One real trap worth flagging for next time: Tailwind v4 auto-prefixes the utility name onto whatever you call the token, so `--color-bg-app` generates `bg-bg-app`, not `bg-app` — caught before it shipped by checking the compiled CSS output directly rather than trusting the naming by eye; token names are the bare semantic role with no `bg-`/`text-`/`border-` prefix of their own. Two shades needed to actually change between themes rather than just moving to the other theme's equivalent tier: `cyan-400`/`red-400`, used as *text* (not button-fill) color, fail contrast against a white background, so light mode uses `cyan-700`/`red-600` for those specific tokens while every other token is a straightforward tier remap.
`src/theme.tsx` (`ThemeProvider`/`useTheme`, React Context — needed because the theme is read in two places with no shared parent short of `App.tsx`: the toggle itself, and `Editor.tsx` for Monaco's theme prop) plus a new inline script in `index.html` (matching the two that already live there for COI/PageConfig setup) that sets `data-theme` synchronously before first paint, reading the *already-set* attribute back in `theme.tsx` rather than duplicating the localStorage read — avoids a flash of the wrong theme on a returning visitor without needing two copies of the same logic. `ThemeToggle.tsx` renders at `App.tsx`'s top level, not inside `Toolbar`, because `UnitIndex` has no header/chrome of its own to host a control and the toggle needs to be reachable from both it and an open unit.
Monaco needed no custom theme work for light/dark — swapping the `theme` prop between its built-in `vs`/`vs-dark` themes was enough, since both already define colors for the standard token categories (`keyword`/`comment`/`string`/`number`) `octaveLanguage.ts`'s tokenizer (T3.14) emits. A custom `monaco.editor.defineTheme` only becomes necessary if/when high contrast lands.
Deliberately left un-tokenized, in both themes: Plotly figure canvases and Monaco's own figure-adjacent chrome (figures render on white always, matching real desktop Octave — pre-existing decision from T3.6/PlotOutput.tsx, unrelated to this ticket), small fixed-size status dots (a mid-tone dot reads fine unchanged against either theme, not worth a token for 2-3 sites), and `shadow-black/50` (shadows work as a constant across themes). One pre-existing inconsistency fixed in passing: `PlotOutput.tsx` had a stray `text-neutral-500` where every other muted-text spot used `slate-500` — now the same `text-muted` token as everywhere else.
*Acceptance:* Toggling actually changes computed styles (not just the `data-theme` attribute), persists across a reload, and Monaco's own theme class swaps with it — verified via `m0-spike-driver/t70-theme-toggle.js`. Screenshotted both themes across the unit index, an open unit (file browser/editor/command window), a figure window, and a confirm dialog (`t71-theme-screenshots.js`) and read the screenshots back directly rather than trusting the class-swap alone — legible and consistent in both themes, no broken contrast, no leftover unthemed elements found in a full repo-wide grep sweep after the migration.
Follow-up fix, same day: the fixed-corner toggle collided with `Toolbar`'s own status pill (also pinned top-right via `ml-auto`) — a `fixed`-positioned element doesn't participate in a flex parent's layout, so the two landed on top of each other instead of side by side. `ThemeToggle` gained a `variant` prop (`'inline'` renders as an ordinary flex child inside `Toolbar`, `'fixed'` keeps the corner placement for `UnitIndex`, which has no header to host a control) — caught from a screenshot the user sent, confirmed fixed via a close-up screenshot of the Toolbar area in both themes before shipping.

**T3.16 — High-contrast theme (added, not in original plan)**
The deferred follow-up from T3.15. Genuine accessibility mode, so every color value was computed against the actual WCAG 2.1 relative-luminance contrast formula during planning — not chosen by eye — targeting 7:1 (AAA) for text and 3:1 (WCAG 1.4.11) for non-text UI. Confirmed with Stephen up front: one high-contrast variant (black background, matching Windows High Contrast Black, the reference convention), not separate black/white options.
Extends T3.15's token architecture with a third `:root[data-theme="high-contrast"]` override block — same 15 tokens, no new mechanism. Pure black for *both* `--color-app` and `--color-surface` (dark mode uses two distinct near-black tiers; high-contrast doesn't) is deliberate: real HC design avoids relying on subtle luminance steps between regions at all, since faint differences are exactly what this mode exists to eliminate — `--color-line` (now pure white, 21:1) carries the structural weight instead. `--color-faint` needed a different value than the dark-mode tier's usual relationship to its background: dark mode's `#475569` only reaches 6.25:1 against pure black, short of the 7:1 AAA target, so high-contrast uses `#a0a0a0` (8.03:1) there instead.
One real change beyond a value swap, found by actually computing the numbers rather than assuming a token swap would be enough: white button labels on filled accent/danger buttons (`text-white`, left alone by T3.15 since it read fine on both cyan-600 and red-600 fills) computes to **1.41:1** against high-contrast's much brighter yellow accent fill — nowhere close to usable. Black text on the same fill is 14.88:1. Added `--color-on-accent` (white in dark/light, black in high-contrast) and swapped the 5 hardcoded `text-white` instances across `Toolbar.tsx`, `ConfirmDialog.tsx`, `PersistenceWarning.tsx`, and `StartupOverlay.tsx`.
Monaco again needed no custom `defineTheme` work: it ships a built-in `hc-black` theme (confirmed by reading `monaco-editor`'s own `themes.js`) with color rules already defined for the same token categories `octaveLanguage.ts` emits. `Editor.tsx`'s theme selection became a 3-way lookup table instead of a ternary.
The 2-state click-to-cycle `ThemeToggle` button was replaced with a native `<select>` (Dark/Light/High Contrast): cycling through 3 states hides the third option behind repeated clicks, which undercuts the point of an accessibility feature needing to actually be discoverable. A native `<select>` is keyboard-navigable and screen-reader-labeled with no custom ARIA work needed, and slots into the same `variant` (`'fixed'`/`'inline'`) placement logic T3.15's toggle-overlap fix already built.
Re-verified the previously-"left unthemed" decorative elements (status dots, loading-bar fill) specifically for this ticket rather than assuming the T3.15 call still held at a stricter bar: all three (`slate-500` 4.41:1, `cyan-400` 11.62:1, `red-400` 7.59:1) clear the 3:1 non-text minimum against pure black with room to spare, so no change was needed there.
*Acceptance:* `m0-spike-driver/t70-theme-toggle.js` extended to cycle all 3 themes via the new `<select>`, confirming computed styles change correctly at each step (including the black-on-yellow button label, not the old white), Monaco swaps to `hc-black`, and the choice persists across reload — 16/16 checks pass. Screenshotted high-contrast across the same 4 views as T3.15 and read them back directly: crisp white borders, correctly black button labels on yellow/red fills, legible syntax highlighting, no washed-out or invisible elements anywhere.

**T3.17 — 8-bit retro and Matrix themes (added, not in original plan)**
Two "fun" themes on top of T3.15/T3.16's now-proven 3-theme architecture, plus emoji prefixes on all 5 dropdown options for at-a-glance recognition. Confirmed with Stephen up front: NES-style specifically for retro (not Game Boy or amber-CRT), and real pixel-font typography for retro's UI chrome rather than just a recolor — Matrix deliberately keeps the existing monospace stack, since a blocky game font would work against its actual identity (green terminal text), not for it.
Bigger than it looks next to T3.15/T3.16: those three themes all reused a Monaco theme Monaco already ships (`vs`/`vs-dark`/`hc-black`); neither retro nor Matrix has a built-in equivalent, so this is the first ticket needing a real `monaco.editor.defineTheme` call. New `src/components/monacoThemes.ts` registers both (`base: 'vs-dark', inherit: true`, overriding only `editor.background`/`editor.foreground` and the `keyword`/`comment`/`string`/`number` token colors `octaveLanguage.ts` emits — a partial override like T3.15's built-in reuse, not a full redefinition like `hc-black` needed), called from `Editor.tsx`'s `beforeMount` alongside the existing `registerOctaveLanguage`, guarded by the same module-level `registered` flag idempotency pattern since `beforeMount` fires on every unit remount.
One genuine surprise while verifying: Monaco's built-in themes (`vs`/`vs-dark`/`hc-black`) put their own name in `.monaco-editor`'s DOM classList, but a custom `defineTheme`-registered theme does not — the classList reflects the theme's `base` (here, `vs-dark`, for both) regardless of which custom theme is actually active, even though the custom color rules genuinely apply underneath (confirmed directly: `.monaco-editor-background`'s computed color was pure black under `matrix` and NES-navy under `nes-retro`, exactly as registered). `t70-theme-toggle.js`'s Monaco-theme check was written against the wrong assumption (that a custom theme's registered name becomes its DOM class, extrapolating from the built-in themes' behavior) and needed fixing to check the actual rendered background color for the two custom themes instead of classList, while keeping the classList check for the three built-ins where it's valid.
Retro's pixel font (`@fontsource/press-start-2p`, self-hosted via npm — same vendoring pattern as every other third-party asset in this app, e.g. Monaco/Plotly, rather than a runtime CDN fetch) is a genuine bitmap font ("works best at 8px/16px and other multiples of 8" per its own docs), not smoothly scalable — applying it to all body text would both look wrong at normal reading sizes and risk real layout breakage from its much-wider-per-character glyphs. New `--font-pixel` token (Tailwind v4's `--font-*` namespace, same auto-generated-utility mechanism as the `--color-*` tokens) defaults to the existing system-font stack everywhere and is overridden to the real pixel font only under `:root[data-theme="retro"]` — making the `font-pixel` className a no-op in every other theme, no conditional logic needed in any component that uses it. Applied to a deliberately small set of short/fixed-length chrome text only (app title in `Playground.tsx`/`UnitIndex.tsx`, `PanelHeader.tsx`'s shared title span, Toolbar's Run Tests/Run File labels) — explicitly not to file names, table contents, dialog body text, or anything variable-length/dense where a wide bitmap font would hurt. Caught and fixed one overreach during implementation before it shipped: `Playground.tsx`'s status-bar span initially wrapped the entire "ENGR-183 Octave Playground — {unit.title}" string, including the variable-length unit title; corrected to wrap only the fixed prefix.
Palettes are not WCAG-verified the way T3.16's are — that rigor is specifically an accessibility-mode thing, not a fun-theme thing — but were checked for basic legibility the same way every theme so far has been, via screenshot review before shipping. Matrix's `danger`/`danger-hover`/`danger-fg` deliberately break from its green monotony into red (matching the films' own "red pill" association) rather than using an on-brand green for destructive actions, which would be actively misleading.
*Acceptance:* `m0-spike-driver/t70-theme-toggle.js` extended to cover all 5 themes — `data-theme`/computed-style changes, localStorage persistence and reload survival, Monaco theme swaps (classList for the 3 built-ins, computed editor background for the 2 custom themes), and `font-pixel` presence/absence checks confirming the pixel font renders under retro and stays off everywhere else, including Matrix — 24/24 checks pass. Screenshotted both new themes across the same 4 views as every prior theme (`t71-theme-screenshots.js`, extended to loop all 5) and read them back directly: legible in both, no broken button/header layout from the pixel font, Matrix's red danger break and retro's blue accent both read correctly against their backgrounds.

**T3.18 — Editor/console font-size controls (added, not in original plan)**
Instructor request, for recording course videos: zooming the whole browser also blows up the toolbar/panel chrome, not just the code/output text that actually needs to be bigger on screen. `src/components/FontSizeControls.tsx` is a small "little A / big A" stepper (matching the ask literally) — two buttons with the current size in px between them, so it stays legible at a glance while narrating. Wired independently into `Editor.tsx` (drives Monaco's `fontSize` option directly, default 13px matching the prior hardcoded value) and `CommandWindow.tsx` (drives a plain inline `style={{ fontSize }}` on the output `<pre>`, default 12px matching `text-xs`'s prior rendered size) — two separate controls, not one global zoom, since an instructor may want the code and the console output at different sizes for a given shot. Each persists to its own `localStorage` key (`engr183-editor-font-size` / `engr183-console-font-size`), read on mount the same way `theme.tsx` reads its initial theme — so the size sticks across a unit switch (`Playground` fully remounts via `key={unit.id}`) and across sessions, without lifting any state up to `App.tsx`. Clamped to 10–32px; the stepper button disables itself at whichever bound is currently active rather than silently no-op'ing, so it's obvious from the control itself when you've hit the limit.
`PanelHeader.tsx` (shared by File Browser/Workspace/Command Window) gained an optional `extra` slot, rendered between the title and the collapse button, so Command Window's stepper didn't need a bespoke header layout — File Browser and Workspace pass nothing and render exactly as before.
*Acceptance:* `m0-spike-driver/t75-font-size-controls.js` (9/9 checks): both controls default to the prior rendered sizes, step by 1px per click in both directions, persist across a full unit-remount navigation, and clamp correctly at the floor with the button disabling itself. Screenshotted at default and at a heavily zoomed-in size (Editor 21px / Console 22px) and read back directly — both scale cleanly with no layout breakage; also checked against the Matrix theme specifically (highest-risk for a muted/faint-text legibility issue) and confirmed legible.

**T3.19 — Interactive command-line in the Command Window, Phase 1 (added, not in original plan)**
Pedagogically motivated: desktop Octave's Command Window lets a student type a one-off command and see the result immediately (checking a variable, testing a one-liner) — the app previously forced even that into "write it in a file, click Run File," a real mismatch with the tool students already use locally. Confirmed with Stephen up front and scoped deliberately in two ways given the size of the change (it touches the execution path Run Tests/Run File/Workspace/figures all share): **single-line commands only** for Phase 1 (multi-line `for`/`if`/`function`...`end` continuation, the way desktop Octave's own prompt handles it, is real complexity deferred to Phase 2), and the Command Window becomes **persistent** — Run Tests/Run File stop wiping it on every run, matching real Octave (only `clc`, or the new Clear button, clears it now). Verified this was safe to build the way `session.ts`'s `execute()` already works rather than needing kernel-level changes: its own comment confirms only one `execute()` call is ever safe in flight at a time (it reassigns the kernel's message-routing callback per call) — a REPL command is just another string handed to the same `execute(code, callback)` Run File already uses, gated by the same `busy` status Toolbar already respects.
`Playground.tsx`: `runCode()` no longer clears `output`/`figures` at the start (the actual behavior change) — a new `clearOutput()` is the one explicit way to wipe them now, wired to Command Window's Clear button and to a typed `clc`/`clc;` at the prompt (handled locally, never sent to the kernel — `clc` has no kernel-state effect in real Octave either, it's purely display). New `handleReplSubmit(command)`: echoes `>> <command>` into `output` first (recreating the terminal-transcript feel, since our input and output are separate elements rather than one real terminal), then reuses `buildWriteFilesCode` + `session.execute()` + `handleExecuteChunk` exactly like `runCode()` already does for Run File, plus a `cd('/engr183/assignments/<unit>')` so a typed command can call a helper function the student just wrote in another file in the unit — the same reachability `run(...)` already gets implicitly for Run File. A crude keyword-balance check (`looksUnclosed`, counting `for`/`if`/`while`/`switch`/`function`/`do`/`try` against their `end`/`endfor`/.../`until` closers — not a real parser, Phase 2 territory if this needs to be exact) catches the common case of typing an unclosed block and shows a friendly redirect ("try Run File for that") instead of surfacing Octave's raw "parse error: unexpected end of input."
`CommandWindow.tsx`: a single-line `<input>` below the output `<pre>` with a fixed `>> ` prompt prefix, styled to match `FileBrowser.tsx`'s existing add-file input convention and driven by the *same* `fontSize` state as the output (T3.18's zoom controls cover the REPL input for free, no new plumbing). Enter submits and pushes onto a local command-history array; Up/Down navigate it (no draft-preservation past the newest entry — noted as a Phase 2 polish item, not built). Disabled with a "Kernel busy…" placeholder whenever a Run Tests/Run File/REPL call is in flight, mirroring Toolbar's existing `disabled={busy}`. A "Clear" button joins the font-size stepper in `PanelHeader`'s `extra` slot (already generic from T3.18, no changes needed there). Output `<pre>` now auto-scrolls to bottom on new content via a ref + effect — not needed before when output was always a single short run's worth of text, but necessary now that it accumulates across an extended session.
Regression risk was the real cost center here, not the new feature itself, since `runCode()`/`handleExecuteChunk` are shared by Run Tests, Run File, the Workspace panel, and figure rendering. Grepped every DESIGN.md-referenced acceptance test for an exact-equality check on output text (the one place persistent-output would silently break an existing assumption) — found none; the closest, `t63-prod-octave-probes.js`, uses `.includes()`, unaffected by concatenation. Two older spike-driver scripts (`t26-plot-render.js`, `t27-scratch.js`) predated T3.5's onboarding overlay and needed their `localStorage` init updated to dismiss it before they'd run at all — an existing gap in test upkeep, not a regression from this ticket, fixed in passing.
*Acceptance:* New `m0-spike-driver/t78-repl.js` (16/16 checks): command echo + result, Workspace panel updates after a REPL command (not just Run Tests/Run File), input disables during any busy window and re-enables after, Up/Down history recall including bottoming out past the newest entry, Clear button and typed `clc` both empty the console, Run File output persists alongside earlier REPL history instead of wiping it, an unclosed block shows the friendly redirect (and a *balanced* single-line block like `for i=1:2, disp(i), end` is correctly not misdetected as unclosed), and the REPL input's font size follows T3.18's existing zoom controls. Full regression sweep before this could be considered safe to merge: `t26-plot-render.js`, `t27-scratch.js`, `t57-rendering-spinner.js`, `t70-theme-toggle.js` (24/24), and `t75-font-size-controls.js` (9/9) all re-run clean against the rebuilt bundle. Screenshotted in dark and Matrix and read back directly — echoed commands, results, the multi-line redirect message, and the Clear button all render correctly in both.
...
**T3.4 — Persistence warning (R5) — DONE**
...
*Acceptance:* Visible before a student invests work — verified via `m0-spike-driver/t43-persistence-warning.js` (shown on first visit, survives backdrop-click and Escape attempts, dismissed only by "Got it," does not reappear after reload once acknowledged) and `t43b-deeplink.js` (same, landing directly on a unit URL, correctly layered above the startup overlay, kernel proceeds normally after dismissal). Copy reviewed and approved by Stephen — no changes requested. Reinforced outside the app too: a course video walks through the same persistence risk, so the in-app warning isn't students' only exposure to it.

**T3.5 — First-run onboarding — DONE**
`src/components/OnboardingOverlay.tsx` is a themed modal (matching `PersistenceWarning`/`ConfirmDialog`/`StartupOverlay`'s existing convention exactly — same `bg-app/85` backdrop, `bg-surface` card, `text-on-accent`-labeled button — so it needed zero new CSS and picks up all 5 themes for free via the existing token system) with a 4-item checklist: the File Browser (where files live, click to open), Run Tests vs. Run File (what each does, framed as "the same as typing `run` at the Octave prompt" — local-Octave vocabulary per the ticket's own framing goal, not notebook/kernel language), how to read the rubric report, and Download File/Download All for Canvas submission. The rubric-report copy (`[ PASS ]`/`[ FAIL ]` tags, the `-> hint` line, the score line) was written by reading `engr183-harness/+engr183/report.m` directly rather than approximating from memory, so it names the exact format a student will actually see, not a paraphrase.
Shown once per browser via its own `localStorage` flag (`engr183-onboarding-seen`), deliberately separate from T3.4's `engr183-persistence-ack` flag despite both being "shown once on first visit" modals: the persistence warning has to reappear if storage was ever cleared, since that's the exact risk it's warning about, but a student who already knows how the UI works still knows it after a storage clear — collapsing them into one flag would have wrongly made the orientation reappear too. `App.tsx` renders it right after the persistence warning is acknowledged (`persistenceAcked && !onboardingSeen`), at the same app-level position/z-tier (`z-[60]`) so it surfaces the same way regardless of whether a student lands on the unit index or deep-links straight into a unit — matching T3.4's own reasoning for that placement. Same "not dismissable via backdrop click or Escape" behavior as `PersistenceWarning`, for the same reason: one short read, one deliberate click through, no accidental dismissal.
*Acceptance:* A student who has only ever used local Octave can complete Unit 1 unaided and recognizes everything they see. Verified via `m0-spike-driver/t72-onboarding.js`: persistence warning shows first on a fresh visit with onboarding withheld until it's acknowledged, onboarding then appears and names the File Browser/Run Tests/rubric-report format/Download controls, survives backdrop-click and Escape attempts, dismisses only via "Let's go," neither modal reappears after a reload, and it surfaces correctly on a direct `?unit=` deep link too. Screenshotted across dark, Matrix, and 8-bit retro and read back directly — legible in all three with no layout issues.

Follow-up, same day: added a "Don't show this again" checkbox (checked by default, matching the original always-once behavior). Unchecking it and dismissing only hides the overlay for the rest of that tab session rather than writing the `engr183-onboarding-seen` localStorage flag, so it comes back on the next full page load for a student who wants the checklist again without digging into browser site-data settings to clear a flag they don't know exists. Considered and deliberately skipped a highlighted-region guided tour (spotlighting the File Browser/Toolbar/Command Window in place) as the alternative to a static checklist: those panels are user-resizable and collapsible (T3.10), so a highlight box would need to track live, user-adjustable DOM positions, and the overlay has to render correctly both on the unit index (where those regions don't exist yet) and mid-kernel-startup on a deep link (before the editor/panels are necessarily settled) — real fragility and cross-theme/cross-layout re-verification cost for a feature whose acceptance bar is just "recognizes everything they see," which the checklist already clears. `t72-onboarding.js` extended to 17 checks: the checkbox is checked by default, an unchecked dismiss hides it for the session but it reappears after reload, and a subsequent checked dismiss persists correctly.
