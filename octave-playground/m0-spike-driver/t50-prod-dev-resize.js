...
(async () => {
  ...
  console.log('dev: resize grew window:', after.width > before.width + 100 && after.height > before.height + 50);
  const plotBox = await page.locator('.js-plotly-plot').boundingBox();
  console.log('dev: plot resized to match window:', Math.abs(plotBox.width - after.width) < 20);

  // Minimize
  await page.locator('[title="Minimize figure"]').click();
  await page.waitForTimeout(300);
  const minimizedBox = await figWindow.boundingBox();
  console.log('dev: window collapsed on minimize:', minimizedBox.height < 50);
  console.log('dev: plot unmounted while minimized:', (await page.locator('.js-plotly-plot').count()) === 0);

  // Expand
  const expandBtn = page.locator('[title="Expand figure"]');
  console.log('dev: expand button present:', (await expandBtn.count()) === 1);
  await expandBtn.click();
  await page.waitForTimeout(500);
  const expandedBox = await figWindow.boundingBox();
  console.log('dev: window restored on expand:', expandedBox.height > 200);
  console.log('dev: plot re-rendered after expand:', (await page.locator('.js-plotly-plot').count()) === 1);
