...
(async () => {
  ...
  console.log('prod: zero phantom windows for no-plot script:', phantomCount === 0);
  const cmdText = await page.evaluate(() => document.querySelector('pre')?.textContent || '');
  console.log('prod: Command Window shows x = 5:', cmdText.includes('x = 5'));
  ...
  console.log('prod: exactly 2 figure windows (not more):', figCount === 2);
  console.log('prod: exactly 2 real rendered plots:', plotCount === 2);
