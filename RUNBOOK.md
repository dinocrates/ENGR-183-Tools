...
not remove its saved copy. Use the row's PNG button for a single image, or its
remove button to delete the saved figure. Reset unit clears saved figures too.

Download All keeps code/text files at the top level and adds a `figures/`
folder containing each figure's `.png` and `.figure.json` files. Keep the JSON
files when moving work between browsers: uploading the ZIP restores the
interactive figures, and their PNGs can be generated again. A PNG by itself
does not contain interactive plot data. Saved figure files are playground
documents, not MATLAB/Octave `.fig` files or a saved Octave workspace.

A successful Run File replaces saved figures from that script, including
removing old figures if the new run produces fewer (or none). Failed or
incomplete runs preserve previous snapshots and add any complete new figures
separately. Command Window updates to a known figure update its saved copy.
Run Tests does not save checker-generated figures. Files excluded by a unit's
submission configuration also exclude figures generated by those files.

Snapshots preserve the plot produced by the code; exploratory zoom/pan in the
viewer is not written back. They are stored in browser storage alongside the
unit's files, so clearing site data removes them. Download All is the portable
copy. An export failure reports an error and does not download a partial ZIP.
Limits: 50 saved figures per unit, 8 MB per figure document; PNG export uses
the original canvas size, bounded to 220–2400 pixels per dimension.
